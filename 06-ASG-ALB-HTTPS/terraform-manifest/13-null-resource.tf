...
resource "null_resource" "name" {
  ...
  connection {
    ...
    private_key = file("${path.module}/private-key/terraform-key-eu-west-2.pem")

  }
  ## File Provisioner: Copies the terraform-key.pem file to /tmp/terraform-key.pem
  provisioner "file" {
    source      = "${path.module}/private-key/terraform-key-eu-west-2.pem"
    destination = "/tmp/terraform-key-eu-west-2.pem"
  }

  ## Remote Exec Provisioner: Using remote-exec provisioner fix the private key permissions on Bastion Host
  provisioner "remote-exec" {
    inline = [
      "chmod 400 /tmp/terraform-key-eu-west-2.pem"
    ]
  }
