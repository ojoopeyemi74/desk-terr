...
resource "aws_cloudwatch_metric_alarm" "app1_asg_cwa_cpu" {
  ...
  ok_actions          = [aws_sns_topic.myasg_sns_topic.arn]  # to trigger SNS via email notification
  alarm_actions     = [
    aws_autoscaling_policy.high_cpu.arn, # to trigger the ASG policy
    aws_sns_topic.myasg_sns_topic.arn # to trigger SNS via email notification
    ]
}
